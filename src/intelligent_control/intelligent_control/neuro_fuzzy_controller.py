...
class NeuroFuzzyNavigation(Node):
    ...
    def waypoint_callback(self, msg):
        ...
        self.get_logger().info(
            f"Successfully processed {len(self.waypoints)} waypoints, current target: "
            f"waypoint {self.current_wp_index} at position "
            f"({self.target_x:.2f}, {self.target_y:.2f}, {self.target_z:.2f}) | "
            f"Vehicle heading: {self.vehicle_heading:.2f}")
    ...
    def check_waypoint_reached(self):
        ...
        target_waypoint = self.waypoints[self.current_wp_index]
        target_x, target_y, _ = target_waypoint['global_coordinates']
        
        if self.distance_to_target_wp < self.waypoint_reached_threshold:
            self._advance_to_next_waypoint(target_x, target_y)

    def _advance_to_next_waypoint(self, prev_x, prev_y):
        """Advance to the next waypoint in the mission."""
        # Store previous waypoint
        self.previous_waypoint = (prev_x, prev_y)

        # Increment waypoint index
        prev_index = self.current_wp_index
        self.current_wp_index += 1

        self.get_logger().debug(f"Reached waypoint {prev_index} with D = {self.distance_to_target_wp}m: ({prev_x}, {prev_y})")
    ...
    def update_navigation_targets(self):
        ...
        Sets target positions, previous waypoint for path calculations,
        and updates path parameters for cross-track error calculations."""
        ...
        # Update path parameters for cross-track error calculations
        self.update_path_parameters()
    ...
    def update_path_parameters(self):
        """Update path parameters for navigation and cross-track error calculation.
        
        Calculates the path line equation, path direction vector, and other
        parameters needed for accurate path following between waypoints."""
        # Validate that we have a valid target waypoint
        if self.current_wp_index >= len(self.waypoints):
            self.get_logger().error("Cannot update path: Invalid waypoint index")
            return False
        
        # Extract the target coordinates
        self.target_x, self.target_y, self.target_z = self.waypoints[self.current_wp_index]['global_coordinates']
        
        # Get the previous waypoint
        prev_x, prev_y = self.previous_waypoint        
        
        # Log path information for debugging
        self.get_logger().debug(
            f"Path updated: from=({prev_x:.2f}, {prev_y:.2f}) to=({self.target_x:.2f}, {self.target_y:.2f}), ")
    ...
    def _create_fuzzy_variables(self):
        ...
        # Output variables - direct motor control as in original code
    ...
    def collect_training_data_from_fuzzy(self):
        ...
        wave_condition = self.fuzzy_variables['wave_condition']
        left = self.fuzzy_variables['left_motor']
        right = self.fuzzy_variables['right_motor']

        # Creating rules
        rules = [
            # Center heading rules
            ctrl.Rule(cte['center'] & distance['very_far'] & wave_condition['stable'], left['very_fast']),
            ctrl.Rule(cte['center'] & distance['very_far'] & wave_condition['stable'], right['very_fast']),

            # Distance-based speed adjustments
            ctrl.Rule(cte['center'] & distance['far'] & wave_condition['stable'], left['fast']),
            ctrl.Rule(cte['center'] & distance['far'] & wave_condition['stable'], right['fast']),

            ctrl.Rule(cte['center'] & distance['medium'] & wave_condition['stable'], left['moderate']),
            ctrl.Rule(cte['center'] & distance['medium'] & wave_condition['stable'], right['moderate']),

            ctrl.Rule(cte['center'] & distance['close'] & wave_condition['stable'], left['slow']),
            ctrl.Rule(cte['center'] & distance['close'] & wave_condition['stable'], right['slow']),

            # Wave compensation - going up
            ctrl.Rule(cte['center'] & distance['very_far'] & wave_condition['going_up'], left['very_fast']),
            ctrl.Rule(cte['center'] & distance['very_far'] & wave_condition['going_up'], right['very_fast']),

            ctrl.Rule(cte['center'] & distance['far'] & wave_condition['going_up'], left['very_fast']),
            ctrl.Rule(cte['center'] & distance['far'] & wave_condition['going_up'], right['very_fast']),

            ctrl.Rule(cte['center'] & distance['medium'] & wave_condition['going_up'], left['very_fast']),
            ctrl.Rule(cte['center'] & distance['medium'] & wave_condition['going_up'], right['very_fast']),

            ctrl.Rule(cte['center'] & distance['close'] & wave_condition['going_up'], left['fast']),
            ctrl.Rule(cte['center'] & distance['close'] & wave_condition['going_up'], right['fast']),

            # Wave compensation - going down
            ctrl.Rule(cte['center'] & distance['very_far'] & wave_condition['going_down'], left['moderate']),
            ctrl.Rule(cte['center'] & distance['very_far'] & wave_condition['going_down'], right['moderate']),

            ctrl.Rule(cte['center'] & distance['far'] & wave_condition['going_down'], left['moderate']),
            ctrl.Rule(cte['center'] & distance['far'] & wave_condition['going_down'], right['moderate']),

            ctrl.Rule(cte['center'] & distance['medium'] & wave_condition['going_down'], left['moderate']),
            ctrl.Rule(cte['center'] & distance['medium'] & wave_condition['going_down'], right['moderate']),

            ctrl.Rule(cte['center'] & distance['close'] & wave_condition['going_down'], left['slow']),
            ctrl.Rule(cte['center'] & distance['close'] & wave_condition['going_down'], right['slow']),

            # Heading correction rules
                # Right turn corrections (reduce right motor speed)
            ctrl.Rule(cte['large_left'], left['very_fast']),
            ctrl.Rule(cte['large_left'], right['slow']),

            ctrl.Rule(cte['left'], left['fast']),
            ctrl.Rule(cte['left'], right['slow']),

                # Right turn corrections (reduce right motor speed)
            ctrl.Rule(cte['right'], left['slow']),
            ctrl.Rule(cte['right'], right['fast']),

            ctrl.Rule(cte['large_right'], left['slow']),
            ctrl.Rule(cte['large_right'], right['very_fast'])]
        ...
        left_motor_speed, right_motor_speed = self.calculate_differential_speeds()
    ...
    def calculate_differential_speeds(self):
        ...
        self.cross_track_error = self.compute_cross_track_error()
    ...
    def custom_backpropagation_step(self, inputs, targets_left_speed, targets_right_speed):
        """perform a single backpropagation step with the given inputs and targets"""

        # convert inputs to PyTorch tensors
        inputs = torch.FloatTensor(inputs).to(self.device)
        targets_left_speed = torch.FloatTensor(targets_left_speed).to(self.device)
        targets_right_speed = torch.FloatTensor(targets_right_speed).to(self.device)
        ...
        # calculate losses
        left_speed_loss = self.criterion(pred_left_speed, targets_left_speed)
        right_speed_loss = self.criterion(pred_right_speed, targets_right_speed)
    ...
    def train_neuro_fuzzy_model(self):
        ...
        y_left = np.array(self.training_data['left_motor_speed']).reshape(-1,1)
        y_right = np.array(self.training_data['right_motor_speed']).reshape(-1,1)

        # convert to PyTorch tensors
        X_tensor = torch.FloatTensor(X)
        y_left_tensor = torch.FloatTensor(y_left)
        y_right_tensor = torch.FloatTensor(y_right)

        # create tensorDataset
        dataset = TensorDataset(X_tensor, y_left_tensor, y_right_tensor)
        ...
        for epoch in range(self.epochs):
            ...
            # loop over batches
            for batch_X, batch_left, batch_right in train_loader:
                # move tensors to the configured device
                batch_X = batch_X.to(self.device)
                batch_left = batch_left.to(self.device)
                batch_right = batch_right.to(self.device)

                # zero the parameter gradients
                self.optimizer.zero_grad()

                # forward pass
                pred_left, pred_right = self.model(batch_X)

                # calculate losses
                left_loss = self.criterion(pred_left, batch_left)
                right_loss = self.criterion(pred_right, batch_right)
                combined_loss = left_loss + right_loss
            ...
            with torch.no_grad():
                # get validation data
                val_X, val_left, val_right = next(iter(val_loader))
                val_X = val_X.to(self.device)
                val_left = val_left.to(self.device)
                val_right = val_right.to(self.device)

                # forward pass
                val_pred_left, val_pred_right = self.model(val_X)

                # calculate losses
                val_left_loss = self.criterion(val_pred_left, val_left).item()
                val_right_loss = self.criterion(val_pred_right, val_right).item()
                val_loss = val_left_loss + val_right_loss
            ...
            self.get_logger().info(f"Epoch {epoch+1}/{self.epochs} - "
                            ...
                            f"Speed Loss: {val_left_loss:.4f}, "
                            f"Heading Loss: {val_right_loss:.4f}, "
    ...
    def compute_navigation_parameters(self):
        ...
        if self.vehicle_state.mode in ['AUTO.MISSION'] and self.vehicle_state.armed and self.current_wp_index < 2: ##self.data_collect_counter <= self.max_samples:
            # get fuzzy logic predictionsn first (always)
        ...
        # if model is trained, use a blend of fuzzy and neural predictions based on confidence
            ...
            while torch.no_grad():
                ...
                pred_left, pred_right = self.model(fuzzified_tensor)
                left_motor_speed = float(pred_left.detach().cpu().numpy()[0][0])
                right_motor_speed = float(pred_right.detach().cpu().numpy()[0][0])

            self.get_logger(f"Neuro-fuzzy preediction: Left/Right: ({left_motor_speed:.2f}/{right_motor_speed:.2f})")
        ...
        self.publish_navigation_commands(left_motor_speed, right_motor_speed)        

        self.get_logger().debug(f"Navigation - Dist: {self.distance_to_target_wp:.2f}m, CTE: {self.cross_track_error:.2f}m, "
        f"Left/Right: {left_motor_speed:.2f} / {right_motor_speed:.2f} m/s")
    ...
    def publish_navigation_commands(self, left_motor_speed, right_motor_speed):
        """sending a command to control the differential steering motors using MAVROS."""
        ...
        # Group 0 is used for direct actutor control
        motor_msg.group_mix = 0
        
        # Control values array: [left_motor, right_motor, 0, 0, 0, 0, 0, 0]
        # Using the first two slots for left and right motor as in the original code
        ...
        self.get_logger().info(f"Navigation - Dist: {self.distance_to_target_wp:.2f}m, CTE: {self.cross_track_error:.2f}m, "
        f"Left/Right: {left_motor_speed:.2f} / {right_motor_speed:.2f} m/s")

        # Log the commands
        self.get_logger().debug(f"Sending differential steering command - Throttle: {left_motor_speed:.2f}, Steering: {right_motor_speed:.2f}")
    ...
    def navigation_callback(self):
        """main timer callback to compute and apply naigation parameters"""
        # Check if we have a mission and if all waypoints have been reached
        if not self.waypoints or self.current_wp_index >= len(self.waypoints):
            return      
        
        # Only execute navigation if we're in the correct mode and armed
        if self.vehicle_state.armed and self.vehicle_state.mode in ["AUTO.MISSION"]:
            self.compute_navigation_parameters()
        else:
            self.get_logger().debug("Vehicle not armed or not in AUTO/GUIDED mode. Navigation on hold.") 
    ...
    def training_timer_callback(self):
        ...
        elif self.model_trained and len(self.training_data['left_motor_speed']) % 100 == 0 and len(self.training_data['left_motor_speed']) > 0:
